...
('Fresh Organic Tomatoes', 'Home farm grown, no pesticide residue, naturally ripened. From Green Field Farm standard greenhouse.', 3.50, 500, 'Green Field Farm, Liangyuan District', '/images/tomato.jpg', '在售', 'Vegetables', (SELECT id FROM users WHERE username='test_merchant1'), 328, 156, '24小时内发货', DATE_SUB(NOW(), INTERVAL 5 DAY), NOW()),
('Free-range Eggs', 'From free-range chickens, orange-red yolk, thick protein. Chickens roam naturally in the orchard.', 15.00, 200, 'Green Field Farm, Liangyuan District', '/images/eggs.jpg', '在售', 'Poultry Eggs', (SELECT id FROM users WHERE username='test_merchant1'), 256, 89, '24小时内发货', DATE_SUB(NOW(), INTERVAL 3 DAY), NOW()),
('Fresh Cucumbers', 'Crisp and refreshing, great for cold salads or cooking. Grown using drip irrigation technology.', 2.00, 800, 'Green Field Farm, Liangyuan District', '/images/cucumber.jpg', '在售', 'Vegetables', (SELECT id FROM users WHERE username='test_merchant1'), 189, 234, '24小时内发货', DATE_SUB(NOW(), INTERVAL 7 DAY), NOW()),
('Organic Bok Choy', 'Tender green leaves, rich in vitamins. No chemical fertilizers, sweet taste.', 2.50, 300, 'Green Field Farm, Liangyuan District', '/images/bokchoy.jpg', '在售', 'Vegetables', (SELECT id FROM users WHERE username='test_merchant1'), 145, 67, '3天内发货', DATE_SUB(NOW(), INTERVAL 2 DAY), NOW()),
('Premium Wheat Flour', 'Refined from quality wheat at Jinsui Cooperative, glutinous and smooth, suitable for steamed buns and noodles.', 3.00, 1000, 'Jinsui Cooperative, Suiyang District', '/images/flour.jpg', '在售', 'Grains', (SELECT id FROM users WHERE username='test_merchant2'), 412, 567, '24小时内发货', DATE_SUB(NOW(), INTERVAL 10 DAY), NOW()),
('Corn Grits', 'Fresh corn ground, golden and delicate, porridge is sweet and delicious. Rich in dietary fiber.', 4.00, 600, 'Jinsui Cooperative, Suiyang District', '/images/corn.jpg', '在售', 'Grains', (SELECT id FROM users WHERE username='test_merchant2'), 298, 345, '24小时内发货', DATE_SUB(NOW(), INTERVAL 8 DAY), NOW()),
('Peanut Oil', 'Physical pressed first-grade peanut oil, golden and transparent, rich fragrance.', 25.00, 150, 'Jinsui Cooperative, Suiyang District', '/images/peanutoil.jpg', '在售', 'Grains', (SELECT id FROM users WHERE username='test_merchant2'), 567, 234, '3天内发货', DATE_SUB(NOW(), INTERVAL 6 DAY), NOW()),
('Sweet Potato Noodles', 'Handmade, crystal clear, boil-resistant and non-muddy. Great for stews and cold dishes.', 12.00, 400, 'Jinsui Cooperative, Suiyang District', '/images/noodles.jpg', '在售', 'Grains', (SELECT id FROM users WHERE username='test_merchant2'), 234, 178, '3天内发货', DATE_SUB(NOW(), INTERVAL 4 DAY), NOW()),
('Fresh Fuji Apples', 'Mountain orchard grown, large and red, sweet and juicy. Natural lighting, top quality.', 5.00, 800, 'Mountain Fruit Orchard, Minquan County', '/images/apple.jpg', '在售', 'Fruits', (SELECT id FROM users WHERE username='test_merchant3'), 678, 456, '24小时内发货', DATE_SUB(NOW(), INTERVAL 1 DAY), NOW()),
('Strawberries', 'Greenhouse strawberries, sweet and delicious, large and plump. On-site picking, fresh guarantee.', 20.00, 100, 'Mountain Fruit Orchard, Minquan County', '/images/strawberry.jpg', '在售', 'Fruits', (SELECT id FROM users WHERE username='test_merchant3'), 890, 123, '预售', DATE_SUB(NOW(), INTERVAL 3 DAY), NOW()),
('Grapes', 'Sunshine Rose grapes, sweet and seedless, thin skin and thick flesh. Premium variety.', 15.00, 300, 'Mountain Fruit Orchard, Minquan County', '/images/grape.jpg', '在售', 'Fruits', (SELECT id FROM users WHERE username='test_merchant3'), 456, 234, '15天内发货', DATE_SUB(NOW(), INTERVAL 9 DAY), NOW()),
('Fresh Peaches', 'Spring Snow peaches, early-maturing variety, tender flesh, juicy and sweet.', 6.00, 500, 'Mountain Fruit Orchard, Minquan County', '/images/peach.jpg', '在售', 'Fruits', (SELECT id FROM users WHERE username='test_merchant3'), 345, 189, '3天内发货', DATE_SUB(NOW(), INTERVAL 6 DAY), NOW());
...
('ORD20260419001', @buyer3_id, @merchant3_id, @product4_id, 8, 40.00, '退单中', '已支付', DATE_SUB(NOW(), INTERVAL 10 DAY), DATE_SUB(NOW(), INTERVAL 8 DAY), '申请中', '退货退款', 'Some bruised apples upon receipt', 3, DATE_SUB(NOW(), INTERVAL 1 DAY), 'YTO Express', 'YT5555666677', DATE_SUB(NOW(), INTERVAL 10 DAY));
...
('Organic Vegetable Base Cooperation', 'Seeking partners to jointly develop 50 mu organic vegetable planting base. We provide land and facilities.', 50, 100000, 50000.00, 'Liangyuan District, Shangqiu', 'Mr. Zhang', '13800138001', '招募中', @merchant1_id, DATE_SUB(NOW(), INTERVAL 20 DAY)),
('Grain Storage Cooperation Project', 'Jinsui Cooperative new granary, seeking grain storage partners, providing storage facilities and sales channels.', 200, 500000, 100000.00, 'Suiyang District, Shangqiu', 'Mr. Li', '13800138002', '招募中', @merchant2_id, DATE_SUB(NOW(), INTERVAL 15 DAY)),
('Fruit Picking Garden Development', 'Mountain Orchard plans to develop 20 mu picking garden, planting cherries and blueberries.', 20, 30000, 30000.00, 'Minquan County, Shangqiu', 'Mr. Wang', '13800138003', '招募中', @merchant3_id, DATE_SUB(NOW(), INTERVAL 10 DAY));
...
(NULL, 'system', 'System', @buyer1_id, 'specific_user', 'Zhang San', 'Order Confirmed Receipt', 'Your order ORD20260420001 has been confirmed receipt.', 'order', (SELECT id FROM orders WHERE order_no='ORD20260420001'), 'Order', DATE_SUB(NOW(), INTERVAL 10 DAY)),
(NULL, 'system', 'System', @merchant1_id, 'specific_merchant', 'Green Field Farm', 'Order Completed', 'Order ORD20260420001 has been confirmed receipt by buyer.', 'order', (SELECT id FROM orders WHERE order_no='ORD20260420001'), 'Order', DATE_SUB(NOW(), INTERVAL 10 DAY)),
(NULL, 'system', 'System', @buyer1_id, 'specific_user', 'Zhang San', 'Order Shipped', 'Your order ORD20260421001 has been shipped. ZTO Express ZT9876543210', 'order', (SELECT id FROM orders WHERE order_no='ORD20260421001'), 'Order', DATE_SUB(NOW(), INTERVAL 2 DAY)),
(NULL, 'system', 'System', @buyer3_id, 'specific_user', 'Wang Wu', 'Return Request Submitted', 'Your return request has been submitted, awaiting review.', 'order', (SELECT id FROM orders WHERE order_no='ORD20260419001'), 'Order', DATE_SUB(NOW(), INTERVAL 1 DAY));
